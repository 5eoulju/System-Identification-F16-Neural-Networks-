function grad = backprop(net, X, Y, Y_est, phi_j, vi)

%{
    Function that applies the error-back propagation method that tries 
    to minimize the cost function traveling down the slope (gradient
    method)
    output: grad = dE/dwt 
%}
...
dvj_dWij = vi; % equal to yi - output of the input layer 
...
%%% Compute grad - dE/dWt 
grad = [reshape(dE_dWij, N_meas, net.N_input*net.N_hidden) dE_dWjk];
